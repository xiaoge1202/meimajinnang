...
- (id)init {
    ...
    if (self) {
        ...
        back.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-40, 182, 38, 28);
...
- (void)backFace{
    ...
    if ( inputString.length ) {
        ...
        if ( stringLength >= FACE_NAME_LEN ) {
            ...
            NSRange range = [string rangeOfString:FACE_NAME_YOU];
            NSRange range1=[string rangeOfString:FACE_NAME_HEAD options:nil range:range];
            if ( range.location == 0 ) {
                
                string = [inputString substringToIndex:
                          [inputString rangeOfString:FACE_NAME_YOU
                                             options:NSBackwardsSearch].location];
//                string=[inputString substringToIndex:[inputString rangeOfString:FACE_NAME_YOU options:[NSBackwardsSearange:<#(NSRange)#>]]
                NSLog(@"====%@",string);
            }
            else {
