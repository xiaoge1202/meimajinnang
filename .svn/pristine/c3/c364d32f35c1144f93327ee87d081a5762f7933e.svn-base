...
- (void)reloadData
{
    ...
    self.scrollView.contentSize = CGSizeMake(self.totalWidth+viewWidth*2, CGRectGetHeight(self.bounds));
    ...
    [self.scrollView setContentOffset:CGPointMake(self.totalWidth / 2-CGRectGetWidth(self.bounds) / 2, 0) animated:YES];
    ...
    for (int i = begin; i<= end;i++) {
        ...
        view.tag = i;
            ...
            //[view removeFromSuperview];
//            self.label = [[UILabel alloc] initWithFrame:CGRectMake((view.frame.size.width-28)/2, (view.frame.size.height-32)/2, 34, 32)];
...
//        {
//            self.scrollView.frame = CGRectMake(CGRectGetWidth(self.bounds)/self.visibleViewCount*2, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds));
//        }
        
    }
    NSLog(@"6666 %lf",self.scrollView.contentOffset.x);
}
