...
-(void)postToServer
{
    ...
    [self.manager GET:circleURL parameters:[@{@"leiid":self.leiid}mutableCopy] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.allArr = [NSJSONSerialization JSONObjectWithData:(NSData *)responseObject options:0 error:nil];
        NSLog(@"全部%@",self.allArr);
...
- (void)viewDidLoad {
    ...
    self.allTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight) style:UITableViewStylePlain];
    self.allTableView.dataSource = self;
    self.allTableView.delegate = self;
    [self.view addSubview:self.allTableView];
    
    
    [self setExtraCellLineHidden:self.allTableView];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.circleNameLabel.text = [[self.allArr objectAtIndex:indexPath.row] objectForKey:@"quanname"];
    
    
    cell.peopleNumImg.image = [UIImage imageNamed:@"人数"];
    cell.peopleNumLabel.text = [[self.allArr objectAtIndex:indexPath.row] objectForKey:@"guanzhuzongshu"];
    
    
    cell.speakNumImg.image = [UIImage imageNamed:@"话题"];
    cell.speakNumLabel.text = [[self.allArr objectAtIndex:indexPath.row] objectForKey:@"tiezizongshu"];
    
    
    quanzi = [[self.allArr objectAtIndex:indexPath.row] objectForKey:@"id"];
...
-(void)saveBtnIsClick:(UIButton *)sender
{
    for (int i = 0; i < [self.allArr count]; i++) {
        if (sender.tag == i) {
            ...
            self.dictionary = [self.allArr objectAtIndex:i];
...
{
    NSDictionary *dic = [self.allArr objectAtIndex:indexPath.row];
