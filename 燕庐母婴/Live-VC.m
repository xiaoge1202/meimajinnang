...
-(void)postToServer
{
    ...
    [self.manager GET:circleURL parameters:[@{@"leiid":self.leiid,@"start":[NSString stringWithFormat:@"%d",jjj],@"userid":_userId}mutableCopy] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (jjj == 0)
        {
            self.liveArr = [[NSMutableArray alloc]initWithArray:self.fenYeArr];
            NSLog(@"%d----liveArr----%@",self.liveArr.count,self.liveArr);
        }
        else
        {
            for (int i = 0; i < [self.fenYeArr count]; i++)
            {
                [self.liveArr addObject:[self.fenYeArr objectAtIndex:i]];
                NSLog(@"%d++++liveArr++++%@",self.liveArr.count,self.liveArr);
            }
...
- (void)viewDidLoad {
    ...
    self.liveTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight-158) style:UITableViewStylePlain];
    ...
    [self setExtraCellLineHidden:self.liveTableView];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.headImg.image = [UIImage imageNamed:@"circleImg"];
     [cell.headImg setImageWithURL:[NSURL URLWithString:[[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"qtupian"]] placeholderImage:[UIImage imageNamed:@"头像"]];
    
    cell.circleNameLabel.text = [[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"quanname"];
    
    
    cell.peopleNumImg.image = [UIImage imageNamed:@"人数"];
    cell.peopleNumLabel.text = [[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"guanzhuzongshu"];
    
    
    cell.speakNumImg.image = [UIImage imageNamed:@"话题"];
    cell.speakNumLabel.text = [[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"tiezizongshu"];
    
    
    quanzi = [[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"id"];
    
    
    [cell.saveBtn addTarget:self action:@selector(saveBtnIsClick:) forControlEvents:UIControlEventTouchUpInside];
    cell.saveBtn.tag = indexPath.row;
    
    if([[[[self.liveArr objectAtIndex:indexPath.row] objectForKey:@"guanzhu"] stringValue]isEqualToString:@"0"]){
...
{
    for (int i = 0; i < [self.liveArr count]; i++) {
        if (sender.tag == i) {
            
            self.dictionary = [self.liveArr objectAtIndex:i];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CircleCell *cell= (CircleCell *)[self.liveTableView cellForRowAtIndexPath:rightindexPath];
    
    NSDictionary *dic = [self.liveArr objectAtIndex:indexPath.row];
...
- (void)setupHeader
{
    ...
    [refreshHeader addToScrollView:self.liveTableView];
...
- (void)setupFooter
{
    ...
    [refreshFooter addToScrollView:self.liveTableView];
