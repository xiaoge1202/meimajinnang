...
#import "PrefixHeader.pch"
#import "UIImageView+AFNetworking.h"
@interface meixingXiang_VC ()<UIScrollViewDelegate,TYAttributedLabelDelegate>
...
@implementation meixingXiang_VC{
    //详情的数据
...
-(void)AFNetworking
{
    ...
    [manager GET:@"http://101.200.234.127:8080/YanLu/meixing/list.do" parameters:[@{@"id":self.ids}mutableCopy] success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)addTextAttributedLabel1
{
    ...
    TYAttributedLabel *label = [[TYAttributedLabel alloc]initWithFrame:CGRectMake(10, 52, CGRectGetWidth(self.view.frame)-20, 0)];
    label.delegate = self;
    [_scrollView addSubview:label];
    _label1 = label;
    
    [label appendText:@"\t"];
    NSString *text = @"总有一天你将破蛹而出，成长得比人们期待的还要美丽。但这个过 程会很痛，会很辛苦，有时候还会觉得灰心。面对着汹涌而来的现实，觉得自己渺小无力。但这，也是生命的一部分，做好现在你能做的，然后，一切都会好的。我们都将孤独地长大，不要害怕.";
   // NSString *text = [[infoArray objectAtIndex:0] objectForKey:@"neirong"];
    ...
    for (NSString *text in textArray) {
        ...
        if (index != a-1) {
            ...
            imageStorage.imageURL =[NSURL URLWithString:@"http://www.ergeshipin.com/uploads/allimg/130326/1-13032609321M92.jpg"];
